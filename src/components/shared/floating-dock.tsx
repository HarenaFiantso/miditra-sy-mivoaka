...
const FloatingDockDesktop = ({
  ...
  return (
    <motion.div
      ...
      className={cn(
        'backdrop-blur-glass mx-auto hidden h-16 items-end gap-4 rounded-full border-r border-white/10 bg-gradient-to-br from-[#0f131a] to-[#181d25] px-4 pb-3 md:flex',
        className
      )}
    >
      {items.map((item) => (
  ...
  return (
    <a href={href}>
      <motion.div
        ...
        className="relative flex aspect-square items-center justify-center rounded-full bg-[#181d25]"
      >
        <AnimatePresence>
          {hovered && (
            <motion.div
              ...
              className="cta-button absolute -top-8 left-1/2 w-fit rounded-md px-2 py-0.5 text-xs whitespace-pre text-white"
            >
              {title}
            </motion.div>
          )}
